...
cycling (Bormann and Likens 1967; see [Site History](SiteHistory.qmd) chapter for details).
...
particulate matter. This grouping includes the base cations (Ca^2+^, Mg^2+^,
K^+^, Na^+^ ) as well as the rock-forming elements (Al, Si, Fe) and the
often growth limiting P (see [Phosphorus](Phosphorus.qmd) chapter). In contrast are
...
predominantly well drained and hence aerated (i.e. high in O~2~), evidence
...
watersheds for comparative and experimental studies (see [Site History](SiteHistory.qmd) chapter for full description). These watersheds encompass a steep hill
...
chemostatic behavior (i.e. invariant concentration) for Mg and SO~4~, and
increased Al concentration with increasing stream discharge (@fig-conc-disch).
For NO~3~ and K the C-D relationships differed greatly between the growing
...
air temperature (see [Climate Change](ClimateChange.qmd) chapter).
...
![Relative growth rate of trees growing in young, mid-age and mature forest plots fertilized for 5 years with (A) phosphorus or (B) nitrogen in comparison with control plots (from Goswami et al. 2018)](figs/Biogeochemistry/RelativeGrowthRate.jpg){#fig-relgrowth}
...
microbial decomposition of litter (see [Decomposition](Decomp.qmd) chapter).
...
and the microbially-mediated decay process (see [Decomposition](Decomp.qmd) chapter).
...
respiratory CO~2~, the C:Nt declines with decay and the switch from
...
cations (H^+^, Al^3+^, Ca^2+^, etc.) and negatively-charged anions (OH^-^,
HCO~3~^-^, SO~4~^2-^, NO~3~^-^). These ions are electrostatically attracted to
...
Al^3+^ \> H^+^ \> Ca^2+^ \> Mg^2+^ \> K^+^ \> NH~4~^+^ \> Na+. Thus, H^+^ will normally displace
...
strong mineral acid anions (SO~4~^2-^, NO~3~^-^, Cl^-^), bicarbonate and carbonate
(HCO~3~^-^, CO~3~^2-^), and organic anions (mostly carboxyl, COO-). To
...
In some forests the principal mobile anion is HCO~3~^-^ generated by
dissolution of respiratory CO~2~ and subsequent dissociation of carbonic
acid, H~2~CO~3~ to H^+^ + HCO~3~^-^. The H^+^ will displace base cations like K^+^,
Ca^2+^ from CEC and transport them through soil as neutral salts. Which
...
soil at HB are SO~4~^2-^ and NO~3~^-^, and variation in base cation leaching is
...
These mineral acid anions, NO~3~^-^ and SO~4~^2-^, are supplied to the soil
solution both by atmospheric deposition ("acid rain"; [Climate Change](ClimateChange.qmd)
chapter) and by internal processes. Most of the SO~4~ is supplied by wet
...
addition to atmospheric deposition, NO~3~ is supplied to soil solution by
the process of nitrification of NH4+ which is carried out by prokaryotic
soil microbes (see [Nitrogen Cycling](Nitrogen.qmd) chapter). Note that this is an
...
limit NO~3~ mobility in soil; although S is also an essential nutrient, it
...
Thus, SO~4~ adsorption increased markedly at HB following forest harvest
...
occurs in organic forms at HB (see [Nitrogen Cycling](Nitrogen.qmd) chapter and
[Phosphorus](Phosphorus.qmd) chapter). Thus, besides being the source of mineral nutrients
...
1980; see [Acid Deposition Effects](AcidDepositionEffects.qmd) chapter). At HB, long-term monitoring
...
10:1 was detected (@fig-kvno3). But what mechanisms might account for
...
years after whole-tree harvest of WS5, K~+~ and NO~3~^-^ exhibited a large,
...
from soil; whereas NO~3~ mobility is regulated mostly by biotic uptake
...
rich) situations, O~2~ serves as the "dump" for respiratory electron
...
Fe, S and N. The mostly well drained soils at HB only rarely reach O~2~
levels at which such biochemical reduction processes would be expected.
However, some evidence indicates that reduction of NO~3~ to gaseous forms
by denitrification may be a qualitatively significant N flux pathway at
HB (see [Nitrogen Cycling](Nitrogen.qmd) chapter).
...
either oxidized (ferric, Fe^+3^) or reduced (ferrous Fe^2+^) form. Despite
the predominance of well-aerated (O~2~ rich) soils at HB, a surprisingly
...
from oxidation. The reduction of ferric iron may occur in O~2~ depleted
...
exudation of organic matter and consumption of O~2~ by roots, interacts
